...
class SARSAAgent(Agent):
    def __init__(self, agent_id: int, n_actions: int, n_states: int, config: dict):
        """
        Initialize a SARSA agent with a Q-table.
    ...
    def train(self):
        ...
        for data in self.training_episodes:
            for t in range(len(data) - 1):
                _, state, _, action, reward, done = data[t]
                _, next_state, _, next_action, _, _ = data[t + 1]

                target = reward + self.discount * self.q[next_state][next_action]
                self.q[state][action] += self.learning_rate * (target - self.q[state][action])

                if self.debug:
                    print(f"t={t}, Q[{state},{action}] -> {self.q[state][action]}")

            # Handle last step
            _, state, _, action, reward, done = data[-1]
            self.q[state][action] += self.learning_rate * (reward - self.q[state][action])

            if self.debug:
                print(f"Final step: Q[{state},{action}] -> {self.q[state][action]}")
