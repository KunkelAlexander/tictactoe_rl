...
class DuellingDeepQAgent(agent.Agent): 
    MODE_BINARY   = 0 
    MODE_TUTORIAL = 1

    def __init__(self, agent_id, n_actions, n_states, learning_rate, discount, exploration, learning_rate_decay, exploration_decay, batch_size, replay_buffer_size, n_eval, target_update_freq): 
        """
        ...
        self.online_model.compile(optimizer=tf.keras.optimizers.Adam(learning_rate=learning_rate),
    ...
    def act(self, state, actions): 
        ...
        # exploit
            ...
            q                  = self.online_model(s, training=False).numpy().flatten()
    ...
    def train(self):
        ...
        if len(self.replay_buffer) >= self.batch_size:
            ...
            targets[np.arange(len(targets)), actions]  = rewards + not_terminal * self.discount * np.max(self.target_model.predict_on_batch(next_states), axis=1)
